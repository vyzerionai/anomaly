    ...
    def append(self, flight_data: pd.DataFrame) -> pd.DataFrame:
        ...
        for feature in self._selected_features:
            if feature.startswith("CHT"):
                sig = flight_data[feature] - np.mean(
                    flight_data[["CHT1", "CHT2", "CHT3", "CHT4", "CHT5", "CHT6"]],
                    axis=1,
                )
            elif feature.startswith("EGT"):
                sig = flight_data[feature] - np.mean(
                    flight_data[["EGT1", "EGT2", "EGT3", "EGT4", "EGT5", "EGT6"]],
                    axis=1,
                )
            else:
...
class OneHotAuxiliaryFeature(BaseAuxiliaryFeature):
    """Augments raw categorical feature with one-hot encoded auxiliary features."""
    ...
    def __init__(self, categorical_columns: Mapping[str, Sequence[str]]):
        ...
        for col in categorical_columns:
            feature_names = [
                self._to_one_hot_column_name(col, feature)
                for feature in categorical_columns[col]
            ]
            self._feature_names.extend(feature_names)

    @staticmethod
    def _to_one_hot_column_name(categorical_column_name: str, categorical_value: str):
...
def extract_categorical_columns(
    flight_data: Mapping[tuple[str, ...], tuple[str, pd.DataFrame]],
) -> Mapping[str, set[str]]:
    categorical_columns = collections.defaultdict(set)
...
def apply_auxiliary_features(
    flights: Dict[
        tuple[str, str], tuple[str, pd.DataFrame, Optional[str], Optional[str]]
    ],
    ...
    for flight_key in flights:
        for auxf in auxiliary_features:
            logging.info(
                "Adding auxiliary feature %s to flight %s" % (auxf, flight_key)
            )
